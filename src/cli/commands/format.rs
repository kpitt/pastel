...
impl ColorCommand for FormatCommand {
    fn run(
        ...
        let output = match format_type.as_ref() {
            ...
            "oklab-lightness" => format!("{:.6}", color.to_oklab().l),
            "oklab-a" => format!("{:.6}", color.to_oklab().a),
            "oklab-b" => format!("{:.6}", color.to_oklab().b),
            "oklch" => color.to_oklch_string(),
            "oklab-chroma" => format!("{:.6}", color.to_oklch().c),
            "oklab-hue" => format!("{:.4}", color.to_oklch().h),
